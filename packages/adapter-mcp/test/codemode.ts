...
import { deferred, pair, type Paired } from "./helpers.js";
...
export async function withMcpServer(m: Mocon, sessionId?: string): Promise<Paired & CodeMode> {
  const cm = codeMode(m);
  ...
  return { ...cm, ...(await pair(server, "agent", sessionId)) };
}

/** The low-level path: the handler behind `Server.setRequestHandler(CallToolRequestSchema, ...)`. */
export async function withLowLevelServer(m: Mocon, sessionId?: string): Promise<Paired & CodeMode> {
  const cm = codeMode(m);
