...
- `otel-code-mode.md` — draft semantic conventions for code-mode execution in OpenTelemetry, where OpenTelemetry is the wire format rather than an export target. It carries the same model, closed vocabularies, provenance rules and capability declaration as `core.md` and `provenance.md`, as two spans a host emits through the OpenTelemetry API. It replaces nothing yet; the rest of this directory still describes the record format. `packages/trace` is the reference emitter for it, on the OpenTelemetry API.
