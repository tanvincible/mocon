...
`core.md` 3 defines `ext` only as an "open map of namespaced keys, `vendor.key`," with no further grammar: it does not say how many segments a key may have, or whether a value may be a nested object. This file does not decide that either. The working convention is: prefix every key with your organization's name or a reverse-domain form, `vendor.key` or `domain.key`, and never emit a bare key with no separator. A bare key has no way to signal who defined it, and will collide across hosts and relays.

One rule governs every recommendation below. `provenance.md`'s field table marks `ext.*` on any record P at baseline (`provenance.md` 3). An extension published under `spec/extensions/` MAY define an `ext.<extension>` entry usable in `host.attested` (`provenance.md` 4) that upgrades to H exactly the `ext` keys that extension documents as host-observed; a host attests it only when every such key it emits is determined at a point the program cannot write through. None of the recurring keys below is covered by a published extension today, so a host cannot upgrade any of them by attesting, even when it is certain of its own observation, for example a real subprocess exit code it read from the OS itself — that would take a future `exit_code`-shaped extension defining the entry. A consumer MUST display and aggregate an `ext` value as program-determined unless it recognizes both the extension and the `attested` entry naming that key as H; absent that, the key name implies nothing about its provenance.

Recurring keys seen across real adaptors, given as illustration, not a registry:

- **Process exit code.** A subprocess or container exit status a subprocess-shaped host captured. `core.md` has no first-class field for this. Recommended shape: `vendor.exit_code`.
- **A credits key.** Cost or billing metering. Cost metering is unresolved for the core spec, so it stays in `ext`. Recommended shape: `vendor.credits_used`, `vendor.credits_remaining`.
