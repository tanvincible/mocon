...
// `MOCON_TIME_LIMIT_MS` as the server's time limit, or the default when it is unset. createServer owns the range and refuses a value it cannot use before anything starts.
const given = process.env["MOCON_TIME_LIMIT_MS"];
const server = createServer(m, given === undefined ? undefined : Number(given));
